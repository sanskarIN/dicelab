...
The package/frontend/Cargo/Tauri source version metadata is now `2.0.14`, but the generated lockfile application metadata must also be regenerated by the repository lockfile workflow before the version gate can pass.
...
package-lock.json top-level           2.0.13 (pending regeneration)
package-lock.json packages[""]         2.0.13 (pending regeneration)
src-tauri/Cargo.lock dicelab package  2.0.13 (pending regeneration)
```
